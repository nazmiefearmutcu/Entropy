...
def test_telemetry_line():
    line = format_telemetry(raw_hz=4323, prev=3.10, snap_drops=99566, spikes=229,
...
def test_telemetry_accel_label_and_dropped():
    line = format_telemetry(raw_hz=10, prev=1.0, snap_drops=0, spikes=0,
                            accel="accelerating", dropped=5)
    assert "● Accelerating" in line and "dropped: 5" in line
    steady = format_telemetry(raw_hz=10, prev=1.0, snap_drops=0, spikes=0,
