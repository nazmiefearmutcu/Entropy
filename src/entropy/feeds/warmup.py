...
def bars_from_ohlcv(ohlcv_rows: Iterable[Any]) -> list[Bar]:
    """Map any OHLCV-like rows to strategy/chart Bars.

    Crypto and equity bars used to differ in the one attribute this needs — the
    crypto fork stamped ``exchange_ts``, the equity fork ``source_ts`` — which is
    why there were two of these functions. The merged schema stamps ``source_ts``
    for both, so one function now serves both asset classes. ``local_ts``
    (ingest time) remains the fallback for a source that stamped nothing.
    """
    out: list[Bar] = []
    for o in ohlcv_rows:
        if getattr(o, "close", None) is None:
            continue
        ts = o.source_ts if getattr(o, "source_ts", None) is not None else o.local_ts
...
async def _fetch_yahoo_bars(symbol: str, interval: str) -> list[Any]:
    """Fetch raw Yahoo OHLCV rows via YahooClient. Module-level indirection so
    tests can monkeypatch it; crocodile is imported LAZILY (the sim path must
...
async def warmup_equity_bars(symbol: str, interval: str = "15m",
    ...
    bars = sorted(bars_from_ohlcv(wanted), key=lambda b: b.ts_ns)
    if interval == "4h":
